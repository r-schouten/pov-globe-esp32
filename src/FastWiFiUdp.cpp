...
uint8_t FastWiFiUDP::beginMulticast(IPAddress a, uint16_t p){
...
int FastWiFiUDP::beginMulticastPacket(){
...
int FastWiFiUDP::available(){
  if(!rx_buffer) return 0;
  return rx_buffer->available();
}

int FastWiFiUDP::read(){
...
int FastWiFiUDP::read(unsigned char* buffer, size_t len){
  return read((char *)buffer, len);
}

int FastWiFiUDP::read(char* buffer, size_t len){
  if(!rx_buffer) return 0;
...
int FastWiFiUDP::peek(){
  if(!rx_buffer) return -1;
  return rx_buffer->peek();
}

void FastWiFiUDP::flush(){
  cbuf *b = rx_buffer;
  rx_buffer = 0;
  delete b;
}

IPAddress FastWiFiUDP::remoteIP(){
  return remote_ip;
}

uint16_t FastWiFiUDP::remotePort(){
  return remote_port;
}
