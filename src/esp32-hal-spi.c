...
void spiWrite3(spi_t * spi, uint32_t *data)
{
    while(spi->dev->cmd.usr); // wait until buffer is finished
    spi->dev->mosi_dlen.usr_mosi_dbitlen =  511;
    spi->dev->miso_dlen.usr_miso_dbitlen = 0;
    memcpy(spi->dev->data_buf,data,16*4);
   	spi->dev->cmd.usr = 1;  //start SPI controller
}
